                ...
                <div class="col-12 mb-3">
                    <div class="card bg-light">
                        ...
                        <form method="GET" action="{{ route('alumni.search') }}" enctype="multipart/form-data">
                            @csrf
                            <input type="text" class="form-control" name="name" placeholder="Név" value="{{ $_GET['name'] }}">
                            <input type="text" class="form-control" placeholder="Collegiumi tagság kezdete" pattern="\d{4}" maxlength="4" name="start_of_membership" value="{{ $_GET['start_of_membership'] }}">
                                    ...
                                    <div class="form-group">
                                        <select name="major" class="form-control" id="majorDropdown">
                                            ...
                                            @foreach ($majors_enum as $major)
                                                <option value="{{ $major }}" {{$_GET['major'] == $major ? 'selected ' : ''}}>{{ $major }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        <select name="further_course" class="form-control" id="furtherCourseDropdown">
                                            ...
                                            @foreach ($further_courses_enum as $further_course)
                                                <option value="{{ $further_course }}" {{$_GET['further_course'] == $further_course ? 'selected ' : ''}}>{{ $further_course }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        <select name="scientific_degree" class="form-control" id="scientificDegreeDropdown">
                                            ...
                                            @foreach ($scientific_degrees_enum as $scientific_degree)
                                                <option value="{{ $scientific_degree }}" {{$_GET['scientific_degree'] == $scientific_degree ? 'selected ' : ''}}>{{ $scientific_degree }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        <select name="research_field" class="form-control" id="researchFieldDropdown">
                                            ...
                                            @foreach ($research_fields_enum as $research_field)
                                                <option value="{{ $research_field }}" {{$_GET['research_field'] == $research_field ? 'selected ' : ''}}>{{ $research_field }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
